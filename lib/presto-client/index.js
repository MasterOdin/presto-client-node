...
var Client = exports.Client = function(args){
    ...
    this.retries = typeof args.retries !== 'undefined' ? args.retries : 5;
...
Client.prototype.request = function(opts, callback) {
    ...
    var req = adapter.request(opts, function(res){
        ...
        res.on('end', function(){
            ...
            {
                error = new Error(data ? "execution error:" + data : 'execution error:invalid response code (' + response_code + ')');
            }
...
Client.prototype.statementResource = function(opts) {
    ...
    var max_retries = typeof opts.retries !== 'undefined' ? opts.retries : this.retries;
    ...
    var fetch = function(uri_obj){
        ...
        current_req = client.request(uri_obj, function(error, code, response){
            ...
            if ([502, 503, 504].includes(code)) {
                if (max_retries && current_retries === max_retries) {
                    clear_timeout();
                    error_callback({
                        message: "query fetch failed due to " + code + " code after " + max_retries + " retries",
                        error: error,
                        code: code
                    });
                    return;
